...
class EarlyFusionNet(nn.Module):
    def __init__(self, n_classes, n_features, hidden_size, dropout, sig_model, img_model, sig_hook, img_hook):
        """
        n_classes (int)
        n_features (int)
        hidden_size (int)
        ...
        self.fc1 = nn.Linear(n_features, hidden_size * 2)
    ...
    def forward(self, X_sig, X_img):
        ...
        # flat_img = torch.flatten(act_img.mean(1), start_dim=1)
        X = torch.cat((act_sig, flat_img), dim=1)
...
def train_early_batch(X_sig, X_img, y, model, optimizer, criterion,
                      gpu_id=None, **kwargs):
    """
...
def early_predict(model, X_sig, X_img):
    """
...
def early_evaluate(model, dataloader, part, gpu_id=None):
    """
    model: Pytorch model
    X (batch_size, 1000, 3) : batch of examples
    y (batch_size,4): ground truth labels_train
    ...
    with torch.no_grad():
        ...
        for i, (X_sig_batch, X_img_batch, y_batch) in enumerate(dataloader):
            ...
            y_pred = early_predict(model, X_sig_batch, X_img_batch)
...
# Validation loss
def early_compute_loss(model, dataloader, criterion, gpu_id=None):
    model.eval()
    with torch.no_grad():
        val_losses = []
        for i, (X_sig_batch, X_img_batch, y_batch) in enumerate(dataloader):
...
def main():
    ...
    # REGISTER HOOKS
    # TODO adapt for resnet - need to check sizes of images
    img_hook = 'conv2d_3'
    sig_hook = 'rnn'
    img_model.conv2d_3.register_forward_hook(get_activation(img_hook))
    ...
    features_size = 128 + img_size[img_hook]

    # LOAD DATA
    train_dataset = EarlyFusionDataset(opt.signal_data, opt.image_data, [17111, 2156, 2163], part='train')
    dev_dataset = EarlyFusionDataset(opt.signal_data, opt.image_data, [17111, 2156, 2163], part='dev')
    test_dataset = EarlyFusionDataset(opt.signal_data, opt.image_data, [17111, 2156, 2163], part='test')
    ...
    model = EarlyFusionNet(4, features_size, opt.hidden_size, opt.dropout,
    ...
    for e in epochs:
        ...
        sensitivity, specificity = early_evaluate(model, dev_dataloader, 'dev', gpu_id=opt.gpu_id)
        val_loss = early_compute_loss(model, dev_dataloader, criterion, gpu_id=opt.gpu_id)
        ...
        if val_loss == np.min(valid_mean_losses):
            ...
            torch.save(model.state_dict(),
                       os.path.join(opt.path_save_model, str(datetime.timestamp(dt)) + 'early_lr_3max_model_val' + str(e.item())))

    # Results on test set:
    matrix = early_evaluate(model, test_dataloader, 'test', gpu_id=opt.gpu_id)
    ...
    plot_losses(epochs, valid_mean_losses, train_mean_losses, ylabel='Loss',
                name='training-validation-loss-early3max-{}-{}-{}-{}-{}-{}-{}'.format(
    ...
    plot(epochs, valid_specificity, ylabel='Specificity',
         name='validation-specificity-early3max-{}-{}-{}-{}-{}-{}-{}'.format(
             img_hook, opt.learning_rate, opt.optimizer, opt.dropout, opt.epochs, opt.hidden_size, opt.batch_size))
    plot(epochs, valid_sensitivity, ylabel='Sensitivity',
         name='validation-sensitivity-early3max-{}-{}-{}-{}-{}-{}-{}'.format(
             img_hook, opt.learning_rate, opt.optimizer, opt.dropout, opt.epochs, opt.hidden_size, opt.batch_size))
